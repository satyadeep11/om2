...
var InviteListService = /** @class */ (function () {
    function InviteListService(_http, gc) {
        this._http = _http;
        this.gc = gc;
    }
    InviteListService.prototype.invitelist = function () {
        ...
        var link = this.gc.link + "/php_api/invite-list.php";
    ...
    InviteListService.prototype.pendinglist = function () {
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        var options = new http_1.RequestOptions({ headers: headers });
        var link = this.gc.link + "/php_api/invite-pending-list.php";
        return this._http.post(link, options).pipe(operators_1.map(function (res) { return res.json(); }));
    };
    InviteListService.prototype.acceptedlist = function () {
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        var options = new http_1.RequestOptions({ headers: headers });
        var link = this.gc.link + "/php_api/invite-accepted-list.php";
        return this._http.post(link, options).pipe(operators_1.map(function (res) { return res.json(); }));
    };
    InviteListService.prototype.searchlist = function (email) {
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        var options = new http_1.RequestOptions({ headers: headers });
        var link = this.gc.link + "/php_api/invite-single-user.php";
        return this._http.post(link, email, options).pipe(operators_1.map(function (res) { return res.json(); }));
    };
    InviteListService.prototype.deleteuser = function (customerid) {
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        headers.append('customerid', customerid.toString());
        var options = new http_1.RequestOptions({ headers: headers });
        var link = this.gc.link + "/php_api/delete-single-user.php";
        return this._http.post(link, options).pipe(operators_1.map(function (res) { return res.json(); }));
    };
